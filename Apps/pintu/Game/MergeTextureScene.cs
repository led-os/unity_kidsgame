...
public class MergeTextureScene : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        totalPlace = GameManager.placeTotal;
        GameManager.placeLevel = 0;
        gamePintu.CleanGuankaList();
        gamePintu.ParseGuanka();
    }
    ...
    void DoMerge()
    {
        ...
        if (indexMerge < UIGameBase.listGuanka.Count)
        {
			textTitle.text = "place= " + indexPlace + " ,indexMerge= " + indexMerge ;
            ItemInfo info = UIGameBase.listGuanka[indexMerge] as ItemInfo;
    ...
    void GotoNextPlace()
    {
        ...
        indexPlace = GameManager.placeLevel;
        if (GameManager.placeLevel >= totalPlace)
        {
