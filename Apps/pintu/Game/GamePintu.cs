...
public class GamePintu : GameBase, IUIPintuBlockDelegate
{
    ...
    public void LayOutGamePic()
    {
        ...
        if (objGamePic != null)
        {
            ...
            {
                w_rect = sizeWorld.x;
                h_rect = sizeWorld.y - heightTopbarWorld - Device.offsetBottomWithAdBannerWord;//  GameManager.main.heightAdWorld;
                pos_y = -heightTopbarWorld / 2 + Device.offsetBottomWithAdBannerWord / 2;
            }
            else
            {
                w_rect = sizeWorld.x;
    ...
    public void ShowItemAnimate()
    {
        ...
        //left or top
        {
            if (Device.isLandscape)
            {
                ...
                h = mainCam.orthographicSize * 2 - Device.offsetBottomWithAdBannerWord - heightTopbarWorld;
                y = -mainCam.orthographicSize;
                y += Device.offsetBottomWithAdBannerWord;
            }
            else
            {
                //top
        ...
        //right or bottom
        {
            ...
            else
            {
                ...
                w = Common.GetCameraWorldSizeWidth(mainCam) * 2;
                x = -Common.GetCameraWorldSizeWidth(mainCam);
                y = -mainCam.orthographicSize + Device.offsetBottomWithAdBannerWord;
