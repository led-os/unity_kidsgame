...
public class UIGameNongChang : UIGameBase
{
    ...
    void LoadGame()
    {
        ...
        //bg
        {
            ...
            Texture2D tex = LoadTexture.LoadFromAsset(info.picBg);
            if (tex)
            {
    ...
    public override int ParseGuanka()
    {
        ...
        string picRoot = Common.GAME_RES_DIR + "/place" + idx + "/";

        JsonData items = root["search_item"];
        for (int i = 0; i < items.Count; i++)
        {
            JsonData item = items[i];
            NongChangItemInfo info = new NongChangItemInfo();
            info.sound = (string)item["sound"];
            info.listSearchItem = new List<object>();
            ...
            for (int j = 0; j < targets.Count; j++)
            {
                ...
                info.listSearchItem.Add(findInfo);
            }
            info.icon = "";
            info.picBg = picRoot + strbgname;


            listGuanka.Add(info);
        }

        JsonData mapitems = root["map_item"];
