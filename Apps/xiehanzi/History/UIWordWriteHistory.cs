...
public class UIWordWriteHistory : UIView, ITableViewDataSource
{
    ...
    void Awake()
    {
        ...
        gameMode = UIGameXieHanzi.GAME_MODE_NORMAL;
        //bg
    ...
    void LoadPrefab()
    {
        ...
        {
            GameObject obj = PrefabCache.main.Load("App/Prefab/History/UIWordWriteHistoryCellItem");
    ...
    void ShowSaveImage(WordItemInfo info)
    {
        ...
        if (gameMode == UIGameXieHanzi.GAME_MODE_NORMAL)
        {
            uiRun.ShowBtnRetry(true);
        }
        if (gameMode == UIGameXieHanzi.GAME_MODE_FREE_WRITE)
        {
    ...
    {
        gameMode = UIGameXieHanzi.GAME_MODE_FREE_WRITE;
        UpdateListSortByDate();
    ...
    void GotoWordWrite(WordItemInfo info)
    {
        ...
        WordItemInfo infoGuanka = game.GetGuankaItemInfo(idx) as WordItemInfo;
        game.ParseGuankaItem(infoGuanka);

        infoGuanka.fileSaveWord = info.fileSaveWord;
    ...
    void UpdateListWord(int idx)
    {
        if ((listSort != null) && (listSort.Count > 0))
        {
            ...
            switch (sortType)
            {
                case SORT_TYPE_WORD:
                    {
                        List<WordItemInfo> listtmp = GetDB().GetItemsOfWord(info.id);
                        listWord.Clear();
                ...
                case SORT_TYPE_DATE:
                    {
                        List<WordItemInfo> listtmp = GetDB().GetItemsOfDate(info.date);
                        listWord.Clear();
                        foreach (WordItemInfo infotmp in listtmp)
                        {
                            listWord.Add(infotmp);
