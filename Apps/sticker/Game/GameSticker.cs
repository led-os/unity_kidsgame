...
public class GameSticker : GameBase
{
    ...
    void Awake()
    {
        ...
        listGuanka = GameLevelParse.main.listGuanka;
        ...
        heightTopbarWorld = Common.CanvasToWorldHeight(mainCam, sizeCanvas, 160);
    }
    ...
    public void LoadGame()
    {
        ...
        Debug.LogFormat("game Start:{0}{1}", LevelManager.main.gameLevel, gameTimeStart);
        ...
        rd.sprite = TextureUtil.CreateSpriteFromTex(tex, border);
        ...
        layoutRight.row = 3;
        layoutRight.col = 3;
        if (GameManager.main.gameMode == GAME_MODE_NORMAL)
        {
            GotoGameGroup(gameGroupIndx);
        }
        if (GameManager.main.gameMode == GAME_MODE_RANDOM)
        {
            GotoGameModeRandom();
        }
    ...
    public void LayOutRandom()
    {
        ...
        //矩形排列
        float h_rect = world_h - heightTopbarWorld - Device.offsetBottomWithAdBannerWord;
        float w_rect = world_w * h_rect / world_h;
        float y_center = ((-world_h / 2 + Device.offsetBottomWithAdBannerWord) + (world_h / 2 - heightTopbarWorld)) / 2;
        ...
        if (Device.isLandscape)
        {
            //左
            {
                Vector2 pt = new Vector2(-w_rect / 2, y_center);
                listPos.Add(pt);
            }
            //上
            {
                float step = w_rect / 3;
                Vector2 pt1 = new Vector2(-w_rect / 2 + step, h_rect / 2 + y_center);
                Vector2 pt2 = new Vector2(-w_rect / 2 + step * 2, h_rect / 2 + y_center);
                listPos.Add(pt1);
                listPos.Add(pt2);
            }
            //右
        ...
        else
        {
            ...
            {
                float step = h_rect / 3;
                Vector2 pt1 = new Vector2(-w_rect / 2, -h_rect / 2 + y_center + step);
                Vector2 pt2 = new Vector2(-w_rect / 2, -h_rect / 2 + y_center + step * 2);
                listPos.Add(pt1);
                listPos.Add(pt2);
            }
            //上
            {
                Vector2 pt = new Vector2(0, h_rect / 2 + y_center);
                listPos.Add(pt);
            }
            //右
            {
        ...
        {
            StickerGameItem item = listItemRight[i];
            GameObject obj = item.obj;
            Vector2 pt = listPos[i];
            float x = pt.x;
            float y = pt.y;
            obj.transform.position = new Vector3(x, y, -10f);
            LimitItemPosition(obj);
        }
    }
    public override void LayOut()
    {
        ...
        heightTopbarWorld = h_topbar_world;
        if (!Device.isLandscape)
        {
            layoutLeft.row = 1;
            layoutLeft.col = 3;
    ...
    //防止超出边界和广告区域
    void LimitItemPosition(GameObject obj)
    {
        ...
        float y_top_limit = 0, y_bottom_limit = 0, x_left_limit, x_right_limit;
        y_top_limit = mainCam.orthographicSize - heightTopbarWorld;
        y_bottom_limit = -mainCam.orthographicSize + Device.offsetBottomWithAdBannerWord;
        x_left_limit = -worldSize.x / 2 + Device.offsetLeftWord;
        x_right_limit = worldSize.x / 2 - Device.offsetRightWord;
        ...
        if ((pos.x - bd.size.x / 2) < x_left_limit)
        {
            pos.x = x_left_limit + bd.size.x / 2;
        }
        if ((pos.x + bd.size.x / 2) > x_right_limit)
        {
            pos.x = x_right_limit - bd.size.x / 2;
        }
             ...
             && (itempos.y > (center.y - size.y / 2)) && (itempos.y < (center.y + size.y / 2)))
                {
                    ...
                    if (infoLeft.id == infoRight.id)
                    {
                        ...
                        if ((listItemLeft.Count != 0) && (GameManager.main.gameMode == GAME_MODE_NORMAL))
                        {
    ...
    public void GotoGameModeRandom()
    {
        ...
        float oft_y = Mathf.Max(h_topbar_world, Device.offsetBottomWithAdBannerWord);
        ...
        StickerGameItem infoGuanka = GameLevelParse.main.GetGuankaItemInfo(level) as StickerGameItem;
        float angle = 0;
        ...
        {
            StickerGameItem info = infoGuankaItem as StickerGameItem;
            w = r_display * 0.7f;
            h = w;
            x = center.x;
            y = center.y;
    ...
    {
        LevelManager.main.GotoNextPlace();
    }
    void StartGame()
    {
