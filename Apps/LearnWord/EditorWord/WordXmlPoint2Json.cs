...
public class WordXmlPoint2Json : MonoBehaviour
{
    ...
    void CopyImageFiles(WordItemInfo infoFrom, WordItemInfo infoTo, Texture2D texOfLetter, Rect rcOfLetter)
    {
        ...
        //Debug.Log("bihua:" + " id=" + infoFrom.id + "Count=" + infoFrom.listImageBihua0.Count);
        //笔顺图片 
        ...
        //pic
        if (!FileUtil.FileIsExist(infoTo.pic))
        {
        ...
        //thumb
        if (!FileUtil.FileIsExist(infoTo.thumbLetter))
        {
            CopyFile(infoFrom.thumbLetter, infoTo.thumbLetter);
        }
        //image  
        if (!FileUtil.FileIsExist(infoTo.imageLetter))
        {
            CopyFile(infoFrom.imageLetter, infoTo.imageLetter);
        }
        //笔顺示意图
        if (!FileUtil.FileIsExist(infoTo.imageBihua))
        {
        ...
        //普通话
        if (FileUtil.FileIsExist(infoFrom.soundPutonghua))
        {
            CopyFile(infoFrom.soundPutonghua, infoTo.soundPutonghua);
        }
        //广东话
        if (FileUtil.FileIsExist(infoFrom.soundGuangdonghua))
        {
    ...
    public void ParserGuankaItemXml(WordItemInfo info)
    {
        ...
        //sound
        string strDirRootSound = "AppCommon/Game/hanziyuan";
        //普通话
        ...
        Debug.Log("info.countBihua=" + info.countBihua);
    ...
    int ParseGuankaXml()
    {
        ...
        string fileName = Common.GAME_RES_DIR + "/image/LetterStructure~ipad.xml";
